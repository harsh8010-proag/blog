        ...
        <div>
            <div className='grid grid-cols-1 lg:grid-cols-4 gap-12 mt-16 '>
                <div className='sm:col-span-4'  >
                    {data?.slice().reverse().map((blog) => <div key={blog.id} className='  grid grid-cols-1 sm:grid-cols-3 gap-6 pb-10 border-b border-gray-200'>
                        {console.log()}
                        <div className='col-span-1 h-[240px] bg-center bg-cover rounded-2xl mt-10 cursor-pointer' onClick={() => navigate(`/blog/${blog.id}`)} style={{ backgroundImage: `url(${blog.image})` }}></div>
                        <div className='col-span-2 flex flex-col justify-center gap-6 '>

                            <h2 className='text-xl sm:text-2xl font-bold cursor-pointer' onClick={() => navigate(`/blog/${blog.id}`)}>{blog.title}</h2>
                            <p className='text-lg text-gray-800'>{blog.description}</p>
                            <div className='flex items-center justify-between'>
                                <div className=' flex   gap-2  cursor-pointer' onClick={() => navigate(`/blog/${blog.id}`)}>
                                ...
                                {isUser &&
                                    <div className='flex gap-2'>
                                        ...
                                        <div onClick={() => onDelete(blog.id)} className='cursor-pointer h-8 w-8 rounded-full bg-gray-200 text-gray-800 flex justify-center items-center'><i class="ri-delete-bin-6-fill"></i></div>
                                    </div>}
                            </div>
                        </div>
                    </div>)}
