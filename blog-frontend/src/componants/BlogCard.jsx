...
const BlogCard = ({ data, isLoading, isUser, onEdit, onDelete, isDeleteLoading }) => {



    const navigate = useNavigate();
    if (isLoading) {
        return (<div className='mt-16 px-8 sm:px-10  lg:px-14'>
            <Loader />
                ...
                <div className='sm:col-span-4 relative'  >
                    {data?.slice().reverse().map((blog) => <div key={blog._id} className='  grid grid-cols-1 sm:grid-cols-3 gap-6 pb-10 border-b border-gray-200 '>
                        ...
                        <div className='col-span-2 flex flex-col justify-center gap-6 '>
                            ...
                            <h2 className='text-xl sm:text-2xl font-bold cursor-pointer line-clamp-3 break-words' onClick={() => navigate(`/blog/${blog._id}`)}>{blog.title}</h2>
                            <p className='text-lg text-gray-800  break-all'>{blog.description}</p>
